<!-- BEGIN PAGE BAR -->
<div class="page-bar">
    <ul class="page-breadcrumb">
        <li><a href="${base}">首页</a><i class="fa fa-angle-right"></i></li>
        <li><a href="javascript:;">产品管理</a><i class="fa fa-angle-right"></i></li>
        <li><span>款式特点编辑</span></li>
    </ul>
</div>

<div class="portlet light portlet-fit">
    <div class="portlet-body">
        <div class="row">
            <div class=" col-md-12">
                <div class="portlet light bordered">
                    <div class="portlet-title" style="border-bottom: none">
                        <div class="caption">
                            <span class="caption-subject font-dark bold uppercase">
                                  <i class="icon-bar-chart font-dark"></i>
                                <span class="caption-subject font-dark bold uppercase">特点列表</span>
                            </span>
                        </div>
                    </div>
                    <div class="portlet-body form">
                        <div class="tabbable-line boxless margin-bottom-20">
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab_3">
                                    <div class="row">
                                        <div class="col-md-6 col-sm-12 responsive-1024">
                                            <div class="form-body">
                                                <div class="row">
                                                    <div class="form-group col-md-12">
                                                        <label class="control-label col-md-3"
                                                               style="text-align: right;">
                                                            产品编码：
                                                        </label>
                                                        <div class="col-md-8">
                                                            <div class="input-icon right">
                                                                <i class="fa"></i>
                                                                <input type="text" class="form-control"
                                                                       name="productCode"
                                                                       id="productCode"/>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="form-group col-md-12">
                                                        <label class="control-label col-md-3" for="state_tagsinput_style"> 替代款式：</label>
                                                        <div class="col-md-8">
                                                            <div style="margin-bottom: 15px">
                                                                <input type="text" class="form-control input-large"
                                                                       placeholder="输入款号后按回车"
                                                                       id="state_tagsinput_style"></div>
                                                            <input type="text" class="form-control input-large test"
                                                                   value="" style="display: none;" id="styleRecommend">

                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="form-group col-md-12">

                                                        <div class="input-icon right">
                                                            <form class="form-horizontal form-bordered">
                                                                <div class="form-body">
                                                                    <div class="form-group last">
                                                                        <label class="control-label col-md-3">款式特点：</label>
                                                                        <div class="col-md-8">
                                                                            <div id="ueditor_input" type="text/plain"
                                                                                 style="width:100%;height:150px;"></div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </form>
                                                        </div>

                                                    </div>
                                                </div>


                                            </div>
                                        </div>
                                        <div class="col-md-6 col-sm-12 responsive-1024" id="proImage" >
                                            <img src="" id="productImage"
                                                 style="max-width: 500px;max-height: 666px;margin-top: -30px;margin-left: 100px; object-fit: cover;"/>
                                        </div>

                                    </div>

                                </div>
                            </div>
                            <div class="modal-footer" style="text-align: center">
                                <button type="button" class="btn btn-outline red" id="saveFeature">确认</button>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

        </div>

    </div>
</div>

</div>

</div>
</div>

<div id="enlarge" class="modal fade" data-backdrop='static' tabindex="-1" data-width="826px">

    <img src="" id="largeimg" style="width: 826px;height: 1075px;"/>
    <div class="modal-footer" style="text-align: center">
        <button type="button" data-dismiss="modal" class="btn btn-outline green">取消</button>
    </div>
</div>
