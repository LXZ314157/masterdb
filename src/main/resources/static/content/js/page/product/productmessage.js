...
var TableDatatablesButtons = function () {
    var initDataTable = function () {
        ...
        var oTable = table.dataTable({
            buttons: [
                {extend: '', className: 'btn blue btn-outline sizeLeadIn', text: '尺寸导入'},
                ...
                {extend: 'colvis', className: 'btn red btn-outline showcolumn', text: '显示隐藏列'},
                {
                    ...
                    className: 'btn blue btn-outline export'
                }
            ],
            ...
            'aoColumns': [
                ...
                },
                {
                    ...
                    "mRender": function (data, type, full) {
                        ...
                        html += "<a href='" + webroot + "/product/editfeature?productCode=" + no + "' class='btn btn-xs purple btn-outline' rel='tooltip' target='_blank' title='特点'><i class='fa fa-link'></i></a>";
                        //未同步过
    ...
    function AddParams(aoData) {
        ...
        aoData.push({"name": "batchProductCode", "value": commonClass.getStringVal($('#batchProductCode').val())});
        ...
        aoData.push({"name": "syncStatus", "value": ""});
        $('#loading').modal('show');
...
$(function () {
    ...
    $("#dataexport").click(function () {
        ...
        var syncrecord = commonClass.replaceAllQuotationMarks($("#syncrecord").val());
        var wave = commonClass.replaceAllQuotationMarks($("#wave").val());
        var band = commonClass.replaceAllQuotationMarks($("#band").val());
        var group = commonClass.replaceAllQuotationMarks($("#group").val());
        var line = commonClass.replaceAllQuotationMarks($("#line").val());
        var natureSeason = commonClass.replaceAllQuotationMarks($("#natureseason").val());
        var workgroup = commonClass.replaceAllQuotationMarks($("#workgroup").val());
        var year = commonClass.replaceAllQuotationMarks($("#year").val());
        var devseason = commonClass.replaceAllQuotationMarks($("#devseason").val());
        var productClass = commonClass.replaceAllQuotationMarks($("#productclass").val());
        var item = [];
        item = {
        ...
        var columns = $("#columnOption").val();
        $("#condition").val(JSON.stringify(item));
        $("#exportForm").attr("action", webroot + "/product/exportpro");
        $("#exportForm").attr("target", "_blank");
        $("#exportForm").attr("method", "POST");
        $("#exportForm").attr("contentType", "application/json;charset=utf-8");
        $("#exportForm").submit();
    });
